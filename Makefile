...
.PHONY: all up objects test-db access-db down export-data
...
export-data:
	@echo "Exporting data from MySQL to CSV"
	docker exec -it $(SERVICE_NAME) mysql -u$(MYSQL_USER) -p$(PASSWORD) -e "source /sql_project/export_tables.sql"
	@echo "Copying exported files to local directory"
	docker cp $(SERVICE_NAME):/var/lib/mysql-files/ventas_export.csv ./export/ventas_export.csv
	docker cp $(SERVICE_NAME):/var/lib/mysql-files/clientes_export.csv ./export/clientes_export.csv
